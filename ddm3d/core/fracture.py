...
class Fracture:
    ...
    def create_rectangular(
        ...
        orientation : Tuple[float, float, float], optional
            Fracture orientation (strike, dip, yaw) in degrees, by default (0, 90, 0)
        ...
        for i in range(n_elements_length):
            for j in range(n_elements_height):
                ...
                local_x = -length/2 + actual_element_length/2 + i * actual_element_length
                local_y = 0.0
                local_z = -height/2 + actual_element_height/2 + j * actual_element_height
    ...
    def create_elliptical(
        ...
        for i in range(n_elements_length):
            for j in range(n_elements_height):
                ...
                ellipse_test = (local_x / (length/2))**2 + (local_z / (height/2))**2
