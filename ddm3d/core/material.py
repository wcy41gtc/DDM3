...
class Material:
    ...
    def __init__(
        ...
        self._bulk_modulus = self._youngs_modulus / (
            3.0 * (1.0 - 2.0 * self._poisson_ratio)
        )
        self._lame_lambda = (
    ...
    def __eq__(self, other) -> bool:
        if not isinstance(other, Material):
            return False
        return (
            abs(self._shear_modulus - other._shear_modulus) < 1e-10
            and abs(self._poisson_ratio - other._poisson_ratio) < 1e-10
            and abs(self._density - other._density) < 1e-10
