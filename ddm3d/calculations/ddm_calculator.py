...
class DDMCalculator:
    ...
    def solve_displacement_discontinuities(
        self, fractures: List[Fracture], use_least_squares: bool = True
    ) -> None:
        """
    ...
    def _solve_single_fracture(
        self, fracture: Fracture, use_least_squares: bool
    ) -> None:
    ...
    def _calculate_influence_coefficients(
        ...
        cos_beta = np.cos(np.deg2rad(receiver.o1))
        sin_beta = np.sin(np.deg2rad(receiver.o1))
        ...
        # Calculate influence coefficients
        coef_slsl = Cr * (
            -sin_gamma * cos_gamma * (2 * J8 - x3 * J10)
            + cos_2gamma * (J6 + material.poisson_ratio * J5 - x3 * J12)
            + sin_gamma * cos_gamma * (-x3 * J16)
        )

        coef_slsh = Cr * (
            -sin_gamma * cos_gamma * (2 * material.poisson_ratio * J9 - x3 * J11)
        ...
        coef_slnn = Cr * (
            -sin_gamma
            * cos_gamma
            * (J6 + (1 - 2 * material.poisson_ratio) * J5 - x3 * J12)
            + cos_2gamma * (-x3 * J16)
            + sin_gamma * cos_gamma * (J6 - x3 * J18)
        )

        coef_shsl = Cr * (
            -sin_gamma * ((1 - material.poisson_ratio) * J9 - x3 * J11)
            + cos_gamma * (-material.poisson_ratio * J7 - x3 * J19)
        )

        coef_shsh = Cr * (
            -sin_gamma * ((1 - material.poisson_ratio) * J8 - x3 * J13)
            + cos_gamma * (J6 + material.poisson_ratio * J4 - x3 * J15)
        )

        coef_shnn = Cr * (
            -sin_gamma * (-(1 - 2 * material.poisson_ratio) * J7 - x3 * J19)
            + cos_gamma * (-x3 * J17)
        )

        coef_nnsl = Cr * (
            sin_gamma**2 * (2 * J8 - x3 * J10)
            - 2 * sin_gamma * cos_gamma * (J6 + material.poisson_ratio * J5 - x3 * J12)
            + cos_gamma**2 * (-x3 * J16)
        )

        coef_nnsh = Cr * (
            sin_gamma**2 * (2 * material.poisson_ratio * J9 - x3 * J11)
            - 2 * sin_gamma * cos_gamma * (-material.poisson_ratio * J7 - x3 * J19)
            + cos_gamma**2 * (-x3 * J17)
        )

        coef_nnnn = Cr * (
            sin_gamma**2 * (J6 + (1 - 2 * material.poisson_ratio) * J5 - x3 * J12)
    ...
    def _calculate_integrals(
        ...
        J6 = (
            ...
            * (x2 - b)
            * (x3**2 + r1**2)
            / (r1 * (x3**2 + (x1 - a) ** 2) * (x3**2 + (x2 - b) ** 2))
            - (x1 - a)
            * (x2 + b)
            * (x3**2 + r2**2)
            / (r2 * (x3**2 + (x1 - a) ** 2) * (x3**2 + (x2 + b) ** 2))
            - (x1 + a)
            * (x2 - b)
            * (x3**2 + r3**2)
            / (r3 * (x3**2 + (x1 + a) ** 2) * (x3**2 + (x2 - b) ** 2))
            + (x1 + a)
            * (x2 + b)
            * (x3**2 + r4**2)
            / (r4 * (x3**2 + (x1 + a) ** 2) * (x3**2 + (x2 + b) ** 2))
        )
    ...
    def _calculate_derivatives(
        ...
        # I,111
        J10 = (
            (-(r1 + x2 - b) * ((x1 - a) ** 2 - r1**2) - (x1 - a) ** 2 * r1)
            / (r1**3 * (r1 + x2 - b) ** 2)
            - (-(r2 + x2 + b) * ((x1 - a) ** 2 - r2**2) - (x1 - a) ** 2 * r2)
            / (r2**3 * (r2 + x2 + b) ** 2)
            - (-(r3 + x2 - b) * ((x1 + a) ** 2 - r3**2) - (x1 + a) ** 2 * r3)
            / (r3**3 * (r3 + x2 - b) ** 2)
            + (-(r4 + x2 + b) * ((x1 + a) ** 2 - r4**2) - (x1 + a) ** 2 * r4)
            / (r4**3 * (r4 + x2 + b) ** 2)
        )
        ...
        # I,311
        J12 = (
            (-(x1 - a) * x3 * (2 * r1 + x2 - b)) / (r1**3 * (r1 + x2 - b) ** 2)
        ...
        # I,122
        J14 = (
            (-(r1 + x1 - a) * ((x2 - b) ** 2 - r1**2) - (x2 - b) ** 2 * r1)
            / (r1**3 * (r1 + x1 - a) ** 2)
            - (-(r2 + x1 - a) * ((x2 + b) ** 2 - r2**2) - (x2 + b) ** 2 * r2)
            / (r2**3 * (r2 + x1 - a) ** 2)
            - (-(r3 + x1 + a) * ((x2 - b) ** 2 - r3**2) - (x2 - b) ** 2 * r3)
            / (r3**3 * (r3 + x1 + a) ** 2)
            + (-(r4 + x1 + a) * ((x2 + b) ** 2 - r4**2) - (x2 + b) ** 2 * r4)
            / (r4**3 * (r4 + x1 + a) ** 2)
        )

        # I,222
        J15 = (
            (-(x2 - b) * x3 * (2 * r1 + x1 - a)) / (r1**3 * (r1 + x1 - a) ** 2)
        ...
        # I,322
        J16 = (
            (-(r1 + x2 - b) * (x3**2 - r1**2) - x3**2 * r1)
            / (r1**3 * (r1 + x2 - b) ** 2)
            - (-(r2 + x2 + b) * (x3**2 - r2**2) - x3**2 * r2)
            / (r2**3 * (r2 + x2 + b) ** 2)
            - (-(r3 + x2 - b) * (x3**2 - r3**2) - x3**2 * r3)
            / (r3**3 * (r3 + x2 - b) ** 2)
            + (-(r4 + x2 + b) * (x3**2 - r4**2) - x3**2 * r4)
            / (r4**3 * (r4 + x2 + b) ** 2)
        )

        # I,233
        J17 = (
            (-(r1 + x1 - a) * (x3**2 - r1**2) - x3**2 * r1)
            / (r1**3 * (r1 + x1 - a) ** 2)
            - (-(r2 + x1 - a) * (x3**2 - r2**2) - x3**2 * r2)
            / (r2**3 * (r2 + x1 - a) ** 2)
            - (-(r3 + x1 + a) * (x3**2 - r3**2) - x3**2 * r3)
            / (r3**3 * (r3 + x1 + a) ** 2)
            + (-(r4 + x1 + a) * (x3**2 - r4**2) - x3**2 * r4)
            / (r4**3 * (r4 + x1 + a) ** 2)
        )
    ...
    def calculate_fiber_response(
        self, fractures: List[Fracture], fibers: List[Fiber]
    ) -> None:
        """
    ...
    def _calculate_channel_response(
        self, channel: Channel, fractures: List[Fracture]
    ) -> None:
        ...
        youngs_modulus = (
            2.0
            * fractures[0].material.shear_modulus
            * (1.0 + fractures[0].material.poisson_ratio)
        )
    ...
    def _calculate_element_contribution(
        ...
        cos_beta = np.cos(np.deg2rad(element.o1))
        cos_2beta = np.cos(np.deg2rad(2 * element.o1))
        sin_beta = np.sin(np.deg2rad(element.o1))
        sin_2beta = np.sin(np.deg2rad(2 * element.o1))
    ...
    def calculate_plane_response(
        self, fractures: List[Fracture], planes: List[Plane]
    ) -> None:
        """
    ...
    def _calculate_node_response(self, node: dict, fractures: List[Fracture]) -> None:
