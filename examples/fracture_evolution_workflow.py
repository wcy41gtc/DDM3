...
from ddm3d import Material, Fracture, Fiber, DDMCalculator, FracturePlotter, FiberPlotter
...
def make_fracture(
    tot_l: float, tot_h: float, dl: float, dh: float,
    c_x: float, c_y: float, c_z: float, sk: float, dp: float, yw: float,
    dsl: float, dsh: float, dnn: float,
    Ssl: float, Ssh: float, Snn: float,
    material: Material
) -> Fracture:
    """
...
def create_fracture_series(
    ...
    if mode == 'opening_mode_base':
        # Opening mode base case - 0 degrees strike
        x = 0.0
        y = 0.0
        z = 0.0
        sk = 0.0
        ...
        for i in range(90):
            fracture = make_fracture(
                profiles['l'][i], profiles['h'][i],
                profiles['dl'][i], profiles['dh'][i],
                x, y, z,  # c_x, c_y, c_z
                sk, dp, yw,  # sk, dp, yw
        ...
        # Opening mode - -30 degrees strike
        x = 0
...
def save_results(
    ...
    for fiber in fibers:
        ...
        if fiber.fiber_id == 1:
             ...
             strain_plot_filename = os.path.join(output_dir, f"{mode}_fiber_{fiber.fiber_id}_EYY_U.png")
             ...
             stress_plot_filename = os.path.join(output_dir, f"{mode}_fiber_{fiber.fiber_id}_EYY_U_Rate.png")
             FiberPlotter.plot_fiber_contour(
                 fiber, 
            ...
            strain_plot_filename = os.path.join(output_dir, f"{mode}_fiber_{fiber.fiber_id}_EZZ_U.png")
            FiberPlotter.plot_fiber_contour(
                fiber, 
                component='EZZ_U', 
                scale=20.0,
            ...
            stress_plot_filename = os.path.join(output_dir, f"{mode}_fiber_{fiber.fiber_id}_EZZ_U_Rate.png")
            FiberPlotter.plot_fiber_contour(
                fiber, 
...
def run_opening_mode_base():
    """Run opening mode base case (0 degrees strike)."""
    print("=" * 60)
...
def run_opening_mode():
    """Run opening mode (-30 degrees strike)."""
    print("=" * 60)
